				...
				<rich:panel headerClass="panel-header-gray-border-yellow-bg" styleClass="panel-none-border" bodyClass="panel-documents-gray-border-none-padding">
					...
					<h:panelGrid columns="2">
						<h:panelGrid columns="3">
							...
							<h:outputText value="使用积分值"></h:outputText><rich:inputNumberSpinner id="shoppingCartIntegrationScore" value="#{shopCart.integrationScore}" minValue="0" maxValue="#{currentUser.userInfo.integration}" step="1" disabled="#{(empty shopCart.cart.inventory) or !(shopCart.integrationChecked)}" immediate="true"><f:convertNumber pattern="0" /><a4j:support event="onchange" reRender="messageOfUseIntegration"></a4j:support></rich:inputNumberSpinner><h:panelGroup><h:commandLink action="#{shopCart.useIntegrationLink}" value="使用积分" rendered="#{!(shopCart.integrationChecked)}" disabled="#{empty shopCart.cart.inventory}" styleClass="button-default-style"></h:commandLink><h:commandLink action="#{shopCart.notUseIntegrationLink}" value="不使用积分" rendered="#{shopCart.integrationChecked}" styleClass="button-default-style"></h:commandLink><h:graphicImage id="integrationCheckImage" value="/css/images/icons/IconYes.gif" rendered="#{shopCart.integrationChecked}"></h:graphicImage><h:outputText id="messageOfUseIntegration" value="您能使用的积分范围是0-#{currentUser.userInfo.integration},当前已使用等值金额为:#{shopCart.integrationScore/10}" rendered="#{shopCart.integrationChecked}"></h:outputText></h:panelGroup>
						</h:panelGrid>
						<h:panelGrid columns="2">
							<h:outputText value="商品总额"></h:outputText><h:outputText value="#{shopCart.cart.inventoryPriceTotal}"/>
							<h:outputText value="商品总额"></h:outputText><h:outputText value="#{shopCart.cart.inventoryPriceTotal}"/>
						</h:panelGrid>
					</h:panelGrid>
				</rich:panel>
