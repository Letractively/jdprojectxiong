...
public class EggCookerProductBean extends PagedBaseBean {
	...
	public EggCookerProductBean() {
		...
		this.secondCategoryCode = HeaderConstants.subMenu10;
		this.setBrandCode("");
	...
	public int getTotalCount() {
		...
		try {
			alist = this.getProductService().getPSBPF1F6ProductByCriteria(primaryCategoryCode, secondCategoryCode, brandCode, priceRange, volumeRange, timeType);
	...
	public DataPage<Product> getDataPage(int startRow, int pageSize) {
		...
		try {
			dataPage = new DataPage<Product>(getTotalCount(), startRow,this.getProductService().getPSBPF1F6ProductByCriteria(primaryCategoryCode, secondCategoryCode, brandCode, priceRange, volumeRange, timeType));
		} catch (ProductException e) {
			e.printStackTrace();
		}
		return dataPage;
