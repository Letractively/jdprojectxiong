    		...
    		<ui:define name="content">
				...
				<legend class="demo_legend"><h:outputText value="#{manageHeaderMenu.menuTitle}"/></legend>
    				...
    				<h:panelGrid id="newpanel" columns="3" rowClasses="table-row" columnClasses="table-one-column,table-two-column,table-three-column" headerClass="page-header" footerClass="table-footer" styleClass="table-background" width="96%">
						<f:facet name="header">
                			<h:outputText value="#{productBrandCreate.newProgressString}" id="newProgress"/>
            			</f:facet>
						<h:outputLabel value="类型名称：" for="brandName"></h:outputLabel>
						<h:inputText value="#{productBrandCreate.newProductBrand.brandName}" id="brandName">
							<rich:beanValidator summary="类型有误"/>
                		</h:inputText>
                		<rich:message for="brandName" />
                		<h:outputLabel value="类型序号：" for="serialNumber"></h:outputLabel>
						<h:inputText value="#{productBrandCreate.newProductBrand.serialNumber}" id="serialNumber" onblur="if(/\D/.test(this.value)){alert('只能输入数字');this.select();}">
                    		<rich:beanValidator summary="序号有误"/>
                    	</h:inputText>
                		<rich:message for="serialNumber" />
                		<h:outputText value=" "></h:outputText>
                			<a4j:commandButton action="#{productBrandCreate.createProductBrand}" reRender="newpanel" value="保存"/>
                		<h:outputText value=" "></h:outputText>
					</h:panelGrid>
					</fieldset>
    			</h:form>
