...
public class ElectricPressureCookerProductBean extends PagedBaseBean {
	...
	public ElectricPressureCookerProductBean() {
		super();
		...
		this.secondCategoryCode = HeaderConstants.subMenu08;
		this.setBrandCode("");
	...
	public int getTotalCount() {
		...
		try {
			alist = this.getProductService().getPSBPF1F6ProductByCriteria(primaryCategoryCode, secondCategoryCode, brandCode, priceRange, volumeRange, controlType);
	...
	public DataPage<Product> getDataPage(int startRow, int pageSize) {
		...
		try {
			dataPage = new DataPage<Product>(getTotalCount(), startRow,this.getProductService().getPSBPF1F6ProductByCriteria(primaryCategoryCode, secondCategoryCode, brandCode, priceRange, volumeRange, controlType));
		} catch (ProductException e) {
			e.printStackTrace();
		}
		return dataPage;
